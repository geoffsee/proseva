...
function getCollectionDescription(code: string): string {
    const descriptions: Record<string, string> = {
        ...
        USCODE: "Consolidation of general and permanent federal laws, organized by subject",
        USCOURTS: "Opinions from federal appellate, district, and bankruptcy courts",
...
const GenerateDocumentSchema = z.object({
    ...
    options: z
        .object({
            citationStyle: z.enum(["bluebook", "apa", "mla", "chicago"]).default("bluebook"),
...
function createResearchRouter() {
    ...
    router.get(
        ...
        async (request: Request) => {
            ...
            console.log(`[ResearchRouter ${requestId}] GET /api/research/opinions/search?q=${query}`);
            ...
            try {
                ...
                if (!getConfig("COURTLISTENER_API_TOKEN")) {
                    console.warn(`[ResearchRouter ${requestId}] COURTLISTENER_API_TOKEN not configured`);
                    return new Response(
                        JSON.stringify({
                            error:
                            ...
                            disclaimer:
                                "This information is for educational purposes only and does not constitute legal advice.",
                            setup: "Get a free API token at https://www.courtlistener.com/sign-in/",
                        }),
                        {
                            status: 503,
                            headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                ...
                if (!response.ok) {
                    if (response.status === 429) {
                        console.error(`[ResearchRouter ${requestId}] CourtListener API rate limit exceeded`);
                        return new Response(
                            JSON.stringify({
                    ...
                    if (response.status === 403) {
                        console.error(`[ResearchRouter ${requestId}] CourtListener API authentication failed`);
                        return new Response(
                            JSON.stringify({
                                error: "API authentication failed. Please check your COURTLISTENER_API_TOKEN.",
                                results: [],
                                disclaimer:
                                    "This information is for educational purposes only and does not constitute legal advice.",
                                setup: "Get a free API token at https://www.courtlistener.com/sign-in/",
                ...
                console.error(`[ResearchRouter ${requestId}] ERROR:`, (err as Error)?.message);
                const isRateLimitError = (err as Error)?.message?.includes("Too many attempts");
                return new Response(
                    JSON.stringify({
    ...
    router.get(
        "/api/research/opinions/:id",
        async (request: Request) => {
            const requestId = request.headers.get("X-Request-Id") || crypto.randomUUID();
            const url = new URL(request.url);
            const opinionId = url.pathname.split("/").pop();

            console.log(`[ResearchRouter ${requestId}] GET /api/research/opinions/${opinionId}`);
            ...
            try {
                ...
                console.log(`[ResearchRouter ${requestId}] Fetching opinion details from CourtListener`);
                ...
                if (!response.ok) {
                    ...
                    if (response.status === 403) {
                        return new Response(
                            JSON.stringify({
                                error: "API authentication failed. Check your COURTLISTENER_API_TOKEN.",
                            }),
                            {
                                status: 403,
                                headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                ...
                const isRateLimitError = (err as Error)?.message?.includes("Too many attempts");
                return new Response(
                    JSON.stringify({ error: sanitizeError(err, "Failed to fetch opinion") }),
                    {
                        status: isRateLimitError ? 429 : 500,
                        headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                    }
    ...
    router.get(
        ...
        async (request: Request) => {
            ...
            console.log(`[ResearchRouter ${requestId}] GET /api/research/dockets/${docketId}`);
            ...
            try {
                ...
                console.log(`[ResearchRouter ${requestId}] Fetching docket details from CourtListener`);
                    ...
                    JSON.stringify({
                        ...
                        parties: (data.parties || []).map((party: any) => ({
                            ...
                            attorneys: (party.attorneys || []).map((atty: any) => ({
                                ...
                                roles: (atty.roles || []).map((r: any) => r.role || r).filter(Boolean),
                            })),
                        })),
                        docketEntries: (data.docket_entries || []).slice(0, 50).map((entry: any) => ({
    ...
    router.get(
        ...
        async (request: Request) => {
            ...
            console.log(`[ResearchRouter ${requestId}] GET /api/research/dockets/search?q=${query}`);
            ...
            try {
                ...
                console.log(`[ResearchRouter ${requestId}] Fetching dockets from CourtListener`);
                ...
                if (!response.ok) {
                    if (response.status === 429) {
                        return new Response(JSON.stringify({ error: "API rate limit exceeded", results: [] }), {
                            status: 429,
                            headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                        });
                    }
                ...
                const results = (data.results || []).map((docket: any) => ({
                    ...
                    dateTerminated: formatDate(docket.dateTerminated || docket.date_terminated),
                ...
                const isRateLimitError = (err as Error)?.message?.includes("Too many attempts");
                return new Response(
                    JSON.stringify({ error: sanitizeError(err, "Failed to search dockets"), results: [] }),
                    {
                        status: isRateLimitError ? 429 : 500,
                        headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                    }
    ...
    router.get("/api/research/courts", async (request: Request) => {
        ...
        try {
            ...
            return new Response(
                JSON.stringify({
                    federalAppellate: federalAppellate.sort((a, b) => a.name.localeCompare(b.name)),
                    federalDistrict: federalDistrict.sort((a, b) => a.name.localeCompare(b.name)),
            ...
            console.error(`[ResearchRouter ${requestId}] ERROR:`, (err as Error)?.message);
            return new Response(JSON.stringify({ error: sanitizeError(err, "Failed to fetch courts") }), {
                status: 500,
                headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
            });
        }
    ...
    router.get(
        ...
        async (request: Request) => {
            ...
            if (!citation || citation.length < 5) {
                return new Response(JSON.stringify({ error: "Invalid citation format", results: [] }), {
                    status: 400,
                    headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                });
            }
            ...
            try {
                ...
                console.log(`[ResearchRouter ${requestId}] Looking up citation: ${citation}`);
                ...
                console.error(`[ResearchRouter ${requestId}] ERROR:`, (err as Error)?.message);
                return new Response(
                    JSON.stringify({ error: sanitizeError(err, "Failed to lookup citation"), results: [] }),
                    {
                        status: 500,
                        headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                    }
        ...
        async (request: Request) => {
            ...
            try {
                ...
                if (!getConfig("LEGISCAN_API_KEY")) {
                    console.warn(`[ResearchRouter ${requestId}] LEGISCAN_API_KEY not configured`);
                    return new Response(
                        JSON.stringify({
                            error:
                ...
                if (!response.ok) {
                    ...
                    if (response.status === 403 || response.status === 401) {
                        return new Response(
                            JSON.stringify({
                                error: "LegiScan API authentication failed. Check your LEGISCAN_API_KEY.",
                                results: [],
                                setup: "Get a free API key at https://legiscan.com/legiscan",
                            }),
                            {
                ...
                const totalCount =
                    typeof summary.count === "number" ? summary.count : parseInt(summary.count || "0", 10);
                const hasMore = totalCount ? page * limit < totalCount : results.length === limit;

                console.log(`[ResearchRouter ${requestId}] LegiScan returned ${results.length} results`);
     ...
     */
    router.get(
        "/api/research/statutes/detail",
        async (request: Request) => {
            const requestId = request.headers.get("X-Request-Id") || crypto.randomUUID();
            const url = new URL(request.url);
            const billId = url.searchParams.get("id") || "";

            console.log(`[ResearchRouter ${requestId}] GET /api/research/statutes/detail?id=${billId}`);
            ...
            try {
                ...
                if (latestText) {
                    ...
                    textDocId = latestText.doc_id?.toString() || latestText.docId?.toString() || "";
                ...
                if (textDocId) {
                    ...
                    if (billTextResponse.ok) {
                        ...
                        if (billTextData.status !== "ERROR") {
                            const textPayload = billTextData.text || billTextData.bill?.text || billTextData;
                ...
                return new Response(
                    JSON.stringify({
                        bill: {
                            ...
                            lastAction: bill.last_action || "",
                            lastActionDate: bill.last_action_date || "",
                            session: bill.session?.name || bill.session?.session_name || bill.session_name || "",
                            url: bill.url || "",
    ...
    router.post("/api/research/cases", async (request: Request) => {
        ...
        try {
            ...
            console.error(`[ResearchRouter ${requestId}] ERROR:`, (err as Error)?.message);
            return new Response(JSON.stringify({ error: sanitizeError(err, "Failed to create case") }), {
                status: 500,
                headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
            });
        }
    ...
    router.get("/api/research/cases", async (request: Request) => {
        ...
        try {
            ...
            console.error(`[ResearchRouter ${requestId}] ERROR:`, (err as Error)?.message);
            return new Response(JSON.stringify({ error: sanitizeError(err, "Failed to list cases") }), {
                status: 500,
                headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
            });
        }
    ...
    router.get(
        ...
        async (request: Request) => {
            ...
            console.log(`[ResearchRouter ${requestId}] GET /api/research/cases/${caseId}`);
            ...
            try {
                ...
                console.error(`[ResearchRouter ${requestId}] ERROR:`, (err as Error)?.message);
                return new Response(JSON.stringify({ error: sanitizeError(err, "Failed to get case") }), {
                    status: 500,
                    headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                });
            }
    ...
    router.put(
        ...
        async (request: Request) => {
            ...
            console.log(`[ResearchRouter ${requestId}] PUT /api/research/cases/${caseId}`);
    ...
    router.delete(
        ...
        async (request: Request) => {
            ...
            console.log(`[ResearchRouter ${requestId}] DELETE /api/research/cases/${caseId}`);
    ...
    // Activate a case
    router.post(
        "/api/research/cases/:id/activate",
        async (request: Request) => {
            const requestId = request.headers.get("X-Request-Id") || crypto.randomUUID();
            const url = new URL(request.url);
            const pathParts = url.pathname.split("/");
            ...
            console.log(`[ResearchRouter ${requestId}] POST /api/research/cases/${caseId}/activate`);
            ...
            try {
                ...
                console.error(`[ResearchRouter ${requestId}] ERROR:`, (err as Error)?.message);
                return new Response(
                    JSON.stringify({ error: sanitizeError(err, "Failed to activate case") }),
                    {
                        status: 500,
                        headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                    }
    ...
    router.post(
        ...
        async (request: Request) => {
            ...
            console.log(`[ResearchRouter ${requestId}] POST /api/research/cases/${caseId}/searches`);
            ...
            try {
                ...
                if (!validated.success) {
                    return new Response(JSON.stringify({ error: "Invalid search request body" }), {
                        status: 400,
                        headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                    });
                }
                ...
                console.error(`[ResearchRouter ${requestId}] ERROR:`, (err as Error)?.message);
                return new Response(
                    JSON.stringify({ error: sanitizeError(err, "Failed to save search") }),
                    {
                        status: 500,
                        headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                    }
    ...
    router.post(
        ...
        async (request: Request) => {
            ...
            console.log(`[ResearchRouter ${requestId}] POST /api/research/cases/${caseId}/summarize`);
            ...
            try {
                ...
                if (!validated.success) {
                    return new Response(JSON.stringify({ error: "Invalid summarize request body" }), {
                        status: 400,
                        headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                    });
                }
...
Respond in JSON format:
{
  ...
  "legalIssues": ["...", "..."]
                ...
                console.error(`[ResearchRouter ${requestId}] ERROR:`, (err as Error)?.message);
                return new Response(
                    JSON.stringify({ error: sanitizeError(err, "Failed to generate summary") }),
                    {
                        status: 500,
                        headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                    }
        ...
        async (request: Request) => {
            ...
            try {
                ...
                if (!allowedTypes.includes(file.type)) {
                    return new Response(
                        JSON.stringify({ error: "Invalid file type. Allowed: PDF, DOCX, TXT" }),
                        {
                            status: 400,
                            headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                        }
                ...
                if (file.size > 10 * 1024 * 1024) {
                    return new Response(JSON.stringify({ error: "File too large. Maximum size: 10MB" }), {
                        status: 400,
                        headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                    });
                }
                ...
                console.error(`[ResearchRouter ${requestId}] ERROR:`, (err as Error)?.message);
                return new Response(
                    JSON.stringify({ error: sanitizeError(err, "Failed to upload document") }),
                    {
                        status: 500,
                        headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                    }
    ...
    router.post(
        ...
        async (request: Request) => {
            ...
            try {
                ...
                if (!validated.success) {
                    return new Response(JSON.stringify({ error: "Invalid analyze request body" }), {
                        status: 400,
                        headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                    });
                }
                ...
                if (attachment.type === "text/plain") {
                    textContent = new TextDecoder().decode(new Uint8Array(attachment.data));
                } else {
                ...
                return new Response(JSON.stringify({ analysis, summary: analysis.substring(0, 500) }), {
                    status: 200,
                    headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                });
            } catch (err) {
                console.error(`[ResearchRouter ${requestId}] ERROR:`, (err as Error)?.message);
                return new Response(
                    JSON.stringify({ error: sanitizeError(err, "Failed to analyze document") }),
                    {
                        status: 500,
                        headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                    }
    ...
    router.get(
        ...
        async (request: Request) => {
            ...
            console.log(`[ResearchRouter ${requestId}] GET /api/research/scholar/search?q=${query}`);
            ...
            try {
                ...
                const results = (data.results || []).map((work: any, index: number) => {
                    ...
                    const pdfLink = work.open_access?.oa_url || work.primary_location?.pdf_url || "";
                ...
                console.log(`[ResearchRouter ${requestId}] OpenAlex returned ${results.length} results`);
        ...
        async (request: Request & { user?: any }) => {
            ...
            try {
                ...
                if (!getConfig("GOVINFO_API_KEY")) {
                    console.warn(`[ResearchRouter ${requestId}] GOVINFO_API_KEY not configured`);
                    return new Response(
                        JSON.stringify({
                            error:
                ...
                if (!response.ok) {
                    ...
                    if (response.status === 401 || response.status === 403) {
                        return new Response(JSON.stringify({ error: "Invalid GovInfo API key", results: [] }), {
                            status: 401,
                            headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                        });
                    }
                ...
                const results = (data.results || []).map((item: any, index: number) => {
                    ...
                    const collectionName = GOVINFO_COLLECTIONS[collectionCode] || collectionCode;
                ...
                console.log(`[ResearchRouter ${requestId}] GovInfo returned ${results.length} results`);
    ...
    router.get(
        ...
        async (request: Request & { user?: any }) => {
            ...
            console.log(`[ResearchRouter ${requestId}] GET /api/research/govinfo/package/${packageId}`);

            try {
                if (!getConfig("GOVINFO_API_KEY")) {
                    return new Response(JSON.stringify({ error: "GovInfo API key not configured" }), {
                        status: 503,
                        headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                    });
                }
                ...
                return new Response(
                    JSON.stringify({
                        package: {
                            ...
                            collectionName: GOVINFO_COLLECTIONS[data.collectionCode] || data.collectionCode,
                            dateIssued: data.dateIssued,
                            lastModified: data.lastModified,
                            category: data.category,
                            branch: data.branch,
                            governmentAuthor: data.governmentAuthor1 || data.governmentAuthor2,
                ...
                console.error(`[ResearchRouter ${requestId}] ERROR:`, (err as Error)?.message);
                return new Response(
                    JSON.stringify({ error: sanitizeError(err, "Failed to fetch package details") }),
                    {
                        status: 500,
                        headers: { "Content-Type": "application/json", "X-Request-Id": requestId },
                    }
    ...
    router.get(
        ...
        async (request: Request) => {
            ...
            return new Response(
                JSON.stringify({
                    collections: Object.entries(GOVINFO_COLLECTIONS).map(([code, name]) => ({
                        code,
                        name,
        ...
        async (request: Request & { user?: any }) => {
            ...
            try {
                ...
                const results = [
                    ...
                    ...organic.slice(0, Math.max(0, limit - localResults.length)).map((item: any, index: number) => ({
    ...
    router.post(
        ...
        async (request: Request) => {
            ...
            try {
                ...
                if (!validated.success) {
                    console.error(`[ResearchRouter ${requestId}] Validation error:`, validated.error);
                    return new Response(
                        JSON.stringify({
                ...
                if (webhook?.url) {
                    ...
                    triggerWebhookWithRetry(webhookConfig, webhookPayload).then((result) => {
                        if (result.success) {
                            console.log(
                                `[ResearchRouter ${requestId}] Webhook triggered successfully`
                            );
                        } else {
                            console.error(`[ResearchRouter ${requestId}] Hook failed: ${result.error}`);
                        }
                    })
                }
