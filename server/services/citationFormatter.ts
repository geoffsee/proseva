...
export function detectSourceType(source: CitationSource): SourceType {
  if (source.constitution || source.amendment || source.article)
    return "constitution";
 ...
 */
export function formatShortCase(
  shortName: string,
  volume: string,
  reporter: string,
  pincite: string,
...
function formatBluebookCase(source: CitationSource): string {
  ...
  const courtStr =
    court && !isUnambiguousReporter(source.reporter || "") ? `${court} ` : "";
  ...
  if (source.subsequentHistory) {
    parts[parts.length - 1] += ",";
    const historyPhrase =
      SUBSEQUENT_HISTORY_ABBREVS[source.subsequentHistory] ||
      source.subsequentHistory;
    const histCite = source.subsequentCitation
