...
This document illustrates how the Virginia law knowledge graph explorer tools flow through the system, from Electron
process startup through chat and research agent tool dispatch, using concrete example scenarios.

---

## 1. Process Startup

When the Electron app launches, it starts three processes: the main Electron window, the ProSeVA server, and the
explorer process. The server receives `EXPLORER_URL` so it knows where to send GraphQL queries.
...
The chat endpoint uses a two-phase architecture to separate tool execution from conversational response generation.
Phase 1 uses `TEXT_MODEL_SMALL` (cheap, fast, good at structured tool use) to run the tool-calling loop. Phase 2 uses
`TEXT_MODEL_LARGE` (better natural language) with a clean context — no tool schemas — to produce the final user-facing
reply.
...
Like the chat endpoint, the research agent uses a two-phase architecture. Phase 1 uses `TEXT_MODEL_SMALL` for
tool-calling, accumulating `toolResults` for the sidebar. Phase 2 uses `TEXT_MODEL_LARGE` with a clean context to
produce the final research memo.
...
Both the chat handler and research agent use the same pattern to route tool calls. Local tools are handled by a switch
statement; explorer tools fall through to a type-guard check.
...
The user asks about Virginia's Freedom of Information Act. Phase 1 (`TEXT_MODEL_SMALL`) uses both the local
`SearchKnowledge` vector search and the explorer's graph tools to gather context. Phase 2 (`TEXT_MODEL_LARGE`) receives
the collected tool results and synthesizes a natural language answer with no tool schemas polluting the context.
...
The research agent combines external legal API searches with the local knowledge graph. Phase 1 (`TEXT_MODEL_SMALL`)
gathers data from external APIs and the explorer, accumulating `toolResults` for the sidebar. Phase 2 (
`TEXT_MODEL_LARGE`) receives the collected results and writes a comprehensive research memo with no tool schemas in
context.
...
When the explorer process is down, explorer tool calls fail gracefully. Phase 1's `TEXT_MODEL_SMALL` receives the error
and adapts by falling back to local tools. Phase 2's `TEXT_MODEL_LARGE` receives the collected results (including the
error) and produces the final answer.
...
sequenceDiagram
    ...
    rect rgb(230, 245, 255)
        ...
        and
            ...
            X --x S: Connection refused (ECONNREFUSED)
...
If the Phase 1 LLM keeps calling tools without stopping, the loop terminates after the maximum number of iterations.
Phase 2 still runs, receiving all accumulated tool results, and produces a coherent response from `TEXT_MODEL_LARGE`.
...
## Independent Eval Questions

1. Explain the difference between legal custody and physical custody in Virginia, and cite
   the exact statute sections.

2. Search your knowledge for Virginia Code sections defining “best interests of the child,”
   then quote the most relevant text.
3. Build a custody strategy for a parent seeking joint legal custody but primary physical
   custody in Virginia.
4. Find the top 3 Virginia statutes I should read before filing for custody pro se.
5. Compare sole legal custody vs joint legal custody in Virginia with practical pros/cons for
   trial.
6. What factors does a Virginia court weigh under § 20-124.3? Give plain-English explanations
   and citations.
7. I’m relocating within Virginia; what legal standard applies to modifying custody and
   visitation?
8. Draft a hearing prep checklist for a Virginia custody case with statute-backed priorities.
9. Search for Virginia law on parental decision-making deadlocks (education/medical) in joint
   custody.
10. Give me a cross-examination outline for a custody hearing focused on best-interest
    factors.
11. Find Virginia statutes and case-law concepts relevant to supervised visitation.
12. What evidence should I gather to support a modification of physical custody in Virginia?
13. Compare emergency custody relief vs regular custody petitions in Virginia procedure.
14. Find legal authority on when a child’s preference is considered in Virginia custody
    decisions.
15. Explain contempt risks for violating a custody order in Virginia and how courts usually
    respond.
16. Search your knowledge for Virginia relocation cases and statutes; summarize strongest
    arguments.
17. Generate a parenting-plan structure likely to survive judicial scrutiny in Virginia.
18. Identify weak points in this claim: “I should get sole custody because the other parent
    is difficult.”
19. Give me a statute-cited rebuttal to “joint custody always means 50/50 time.”
20. Find the most relevant Virginia sections for custody, visitation, mediation, and
    modification in one map.
