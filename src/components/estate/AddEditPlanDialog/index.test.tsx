...
describe("AddEditPlanDialog", () => {
  it("renders correctly when open for new plan", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "New Estate Plan" }),
    ).toBeInTheDocument();
    expect(
      screen.getByPlaceholderText("e.g. My Estate Plan 2025"),
    ).toBeInTheDocument();
    expect(
  ...
  it("renders correctly when open for edit", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Edit Estate Plan" }),
    ).toBeInTheDocument();
    expect(
  ...
  it("calls onFormChange when fields change", () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("e.g. My Estate Plan 2025"), {
      target: { value: "Plan 2026" },
    });
    expect(onFormChange).toHaveBeenCalledWith(
      expect.objectContaining({ title: "Plan 2026" }),
    );
    ...
    if (dateInput) {
      ...
      expect(onFormChange).toHaveBeenCalledWith(
        expect.objectContaining({ testatorDateOfBirth: "1980-01-01" }),
      );
    }

    fireEvent.change(inputs[1], { target: { value: "John Doe" } });
    expect(onFormChange).toHaveBeenCalledWith(
      expect.objectContaining({ testatorName: "John Doe" }),
    );

    fireEvent.change(inputs[2], { target: { value: "123 Main St" } });
    expect(onFormChange).toHaveBeenCalledWith(
      expect.objectContaining({ testatorAddress: "123 Main St" }),
    );

    fireEvent.change(inputs[3], { target: { value: "Jane Doe" } });
    expect(onFormChange).toHaveBeenCalledWith(
      expect.objectContaining({ executorName: "Jane Doe" }),
    );

    fireEvent.change(inputs[4], { target: { value: "555-0101" } });
    expect(onFormChange).toHaveBeenCalledWith(
      expect.objectContaining({ executorPhone: "555-0101" }),
    );

    fireEvent.change(inputs[5], { target: { value: "jane@example.com" } });
    expect(onFormChange).toHaveBeenCalledWith(
      expect.objectContaining({ executorEmail: "jane@example.com" }),
    );

    fireEvent.change(inputs[6], { target: { value: "Bob Smith" } });
    expect(onFormChange).toHaveBeenCalledWith(
      expect.objectContaining({ guardianName: "Bob Smith" }),
    );

    fireEvent.change(inputs[7], { target: { value: "555-0202" } });
    expect(onFormChange).toHaveBeenCalledWith(
      expect.objectContaining({ guardianPhone: "555-0202" }),
    );
