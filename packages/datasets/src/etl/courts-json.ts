...
const createLogger = (level: LogLevel = "info"): Logger => ({
  ...
  info(...args) {
    if (["error", "warn", "info"].includes(this.level))
...
const RE_PAGE_HEADER =
  /^(COURT\/DISTRICT\/ADDRESS|JUDGE\(S\)|CLERK\/CONTACT INFORMATION|District Courts Directory|Page\s+\d+(\s+of)?(\s+\d+)?|\d{2}-\w{3}-\d{4}\s+\d{2}:\d{2}:\d{2}|[A-Z]{3}-\d{4}\s+\d{2}:\d{2}:\d{2})$/;
...
export function splitIntoBlocks(text: string): string[][] {
  log.info(
    `Splitting text into blocks (${text.split(/\r?\n/).length} raw lines)`,
  );
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    if (RE_HOMEPAGE.test(line)) {
      blockCount++;
      log.debug(
        `Block #${blockCount} completed (${current.length} lines) → Homepage: ${line}`,
      );
      blocks.push(current);
      current = [];
    }
  ...
  if (current.length > 0) {
    log.warn(
      `Incomplete final block (${current.length} lines) — no Homepage line found`,
    );
    blocks.push(current);
  }
...
export function parseBlock(
  ...
  if (!nameMatch) {
    log.warn(
      `Block #${blockIndex} rejected — no valid court name match in ${lines.length} lines. First line was: "${lines[0]}"`,
    );
    return null;
  }
  ...
  log.debug(
    `Parsing block #${blockIndex} — identified court name: "${firstLine}"`,
  );
  ...
  for (const line of lines.slice(firstLineIndex + 1)) {
    ...
    const isLikelyAddress =
      RE_PO_BOX.test(line) ||
      /^\d+\s/.test(line) ||
      /Street|Avenue|Road|Blvd|Drive|Suite|Floor|Courthouse|Mall|Center|Plaza|Building|Rd\.|St\.|Ave\./i.test(
        line,
      ) ||
