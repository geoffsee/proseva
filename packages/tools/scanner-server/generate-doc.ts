...
async function generateVirginiaLegalPDF(
  ...
  for (const token of tokens) {
    switch (token.type) {
      case "paragraph": {
        ...
        const isLettered =
          segments.length > 0 && /^[a-f]\.\s/.test(segments[0].text);
        ops.push({
          kind: "rich-text",
          segments,
  ...
  for (let p = 0; p < preset.pages || opIndex < ops.length; p++) {
    ...
    if (opIndex >= ops.length && preset.hasAttorneySignature) {
      ...
      page.drawText(`${info.attorney}, Of Counsel`, {
        x: sigX,
        y: sigY - 6,
        size: 11,
        font,
      ...
      sigY -= SLS;
      page.drawText(`Telephone: ${info.phone}`, {
        x: sigX,
        y: sigY - 6,
        size: 11,
        font,
      ...
      sigY -= SLS;
      page.drawText(`Email: ${info.email}`, {
        x: sigX,
        y: sigY - 6,
        size: 11,
        font,
  ...
  // ─── Certificate of Service (required per guidelines) ──────
  {
    ...
    cy -= SLS;
    certPage.drawText(info.defendantAddress || "[Address]", {
      x: 200,
      y: cy,
      size: 12,
      font,
    ...
    cy -= SLS;
    certPage.drawText(info.defendantCityStateZip || "[City, State ZIP]", {
      x: 200,
      y: cy,
      size: 12,
      font,
  ...
  const filename = path.join(
    INBOX_DIR,
    `${vaLegalConfig.outputFilePrefix}${documentType.replace(/-/g, "_")}.pdf`,
  );
...
const allDocTypes = Object.keys(
  vaLegalConfig.documentPresets,
) as DocumentType[];

const LAST_TYPE_FILE = path.join(__dirname, ".last-doc-type");
let lastType: string | null = null;
try {
  lastType = fsSync.readFileSync(LAST_TYPE_FILE, "utf-8").trim();
} catch {}
