...
import type { DbSecurityStatus } from "../../lib/api";
...
export function PassphraseGate({
  ...
  const isTestMode = import.meta.env.MODE === "test" && !forceInteractive;
  const [state, setState] = useState<GateState>("loading");
  const [securityStatus, setSecurityStatus] = useState<DbSecurityStatus | null>(
    null,
  );
  ...
  const handleApplyRecoveryKey = async () => {
    ...
    try {
      const result = await securityApi.applyRecoveryKey(key);
      ...
      setRecoverySuccess("Recovery key applied. Continue with passphrase.");

      if (result.status.passphraseConfigured) {
        setState("enter-passphrase");
      } else {
        setState("create-passphrase");
      }
                ...
                <VStack align="stretch" gap={3}>
                  ...
                  {recoverySuccess && (
                    <Alert.Root status="success">
                      <Alert.Indicator />
                      <Alert.Description>{recoverySuccess}</Alert.Description>
                    </Alert.Root>
                  )}
                  ...
                  <Input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") void handleApplyRecoveryKey();
                    }}
                  />
