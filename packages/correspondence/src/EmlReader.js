...
export class EmlReader {
  ...
  getAttachments() {
    let attachments = [],
      mixedPart = this.#multipartParser.getPartByContentType(
        "multipart",
        "mixed",
      );
    ...
    if (mixedPart) {
      ...
      const att = this.#multipartParser.getPartByContentType(
        "application",
        "octet-stream",
      );
  ...
  getMessageText() {
    ...
    if (html && !html.isAttachment) {
      let htmlStr = html.getBody(),
        hIndex = htmlStr.indexOf("<body");
      if (hIndex !== -1) {
